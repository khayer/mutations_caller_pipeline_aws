class BwaCaller
  ...
  def self.call_paired_end(r1, r2, sai1, sai2,  sam_file, index, log_file, bwa, samtools, job_prefix,account, debug)
    dummy = "\\\\\\"
    cmd = "qsub -o #{log_file} -e #{log_file}_errors -hold_jid bwa_aln_#{job_prefix} -V -cwd -b y -N bwa_#{job_prefix} -l h_vmem=6G -pe make 3  #{account}\
           #{bwa} sampe  #{index}   \
  ...
  def self.call_aln(read, index, out_file, log_file, bwa, job_prefix, account,debug)
    cmd = "qsub -o #{log_file} -e #{log_file}_errors -V -cwd -b y -N bwa_aln_#{job_prefix} -l h_vmem=4G #{account} \
