class BwaCaller
  ...
  def self.call_mem(options)
    if options[:lsf]
      cmd = "bsub -M 30000 -n #{options[:threads]} -o #{options[:log_file]}_bwa_mem_o.log -e #{options[:log_file]}_bwa_mem_e.log -J bwa_#{options[:job_number]} #{options[:bwa]} mem -t #{options[:threads]} #{options[:bwa_index]} #{options[:mutant_r1]} #{options[:mutant_r2]} \\> #{options[:sam_file]}"
    else
