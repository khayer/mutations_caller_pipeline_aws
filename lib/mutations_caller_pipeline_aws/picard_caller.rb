class PicardCaller
  ...
  def self.convert(sam_file, bam_file, picard_tools, log_file, job_prefix, account)
    cmd = "qsub -o #{log_file} -e errors_#{log_file} -V -cwd -b y -hold_jid bwa_#{job_prefix} -N convert_#{job_prefix} -l h_vmem=2G #{account} \
  ...
  def self.rg_and_sorting(bam_file, bam_file_sorted, picard_tools, library, index, sample_name, log_file, id, job_prefix, account)
    cmd = "qsub -o #{log_file} -e errors_#{log_file} -V -cwd -b y -hold_jid convert_#{job_prefix} -N sort_#{job_prefix} -l h_vmem=4G #{account} \
  ...
  def self.mark_dublicates(bam_file_sorted, bam_file_sorted_dublicates, dublicate_metrcis, picard_tools, log_file, job_prefix, account)
    cmd = "qsub -o #{log_file} -e errors_#{log_file} -V -cwd -b y -hold_jid sort_#{job_prefix} -N dublicates_#{job_prefix} -l h_vmem=2G #{account} \
  ...
  def self.build_index(bam_file_sorted_dublicates, picard_tools, log_file, job_prefix, account)
    cmd = "qsub -o #{log_file} -e errors_#{log_file} -V -cwd -b y -hold_jid dublicates_#{job_prefix} -N index_#{job_prefix} -l h_vmem=2G #{account} \
