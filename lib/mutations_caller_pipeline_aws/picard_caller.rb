...
class PicardCaller < Caller
  def initialize(options)
    ...
    @bam_file_sorted = options[:bam_file_sorted]
    ...
    @dublicate_metrcis = options[:dublicate_metrcis]
  end
  ...
  def self.mark_dublicates()
    cmd = "qsub -o #{@log_file} -e #{@log_file}_duplicates_errors -V -cwd -b y \
     ...
     M=#{@dublicate_metrcis} AS=true VALIDATION_STRINGENCY=LENIENT"
  end
