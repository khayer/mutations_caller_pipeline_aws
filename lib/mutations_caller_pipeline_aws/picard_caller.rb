class PicardCaller < Caller
  def initialize(options)
    super(options)
    @sam_file = options[:sam_file]
    @bam_file = options[:bam_file]
    @picard_tools = options[:picard_tools]
  end
  ...
  def sam_to_bam()
    cmd = "qsub -o #{@log_file} -e #{@log_file}_sam_to_bam_errors -V -cwd -b y \
     -hold_jid bwa_#{@job_prefix} -N sam_to_bam_#{@job_prefix} -l h_vmem=7G #{@account} \
     java -Xmx3g -jar  #{picard_tools}/SamFormatConverter.jar I=#{@sam_file} \
     O=#{@bam_file} VALIDATION_STRINGENCY=LENIENT"
  end
  ...
  def self.rg_and_sorting(bam_file, bam_file_sorted, picard_tools, library, index, sample_name, log_file, id, job_prefix, account)
    cmd = "qsub -o #{log_file} -e #{log_file}_rg_sorting_errors -V -cwd -b y -hold_jid sam_to_bam_#{job_prefix} -N sort_#{job_prefix} -l h_vmem=7G #{account} \
