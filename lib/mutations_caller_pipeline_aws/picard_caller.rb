class PicardCaller
  ...
  def self.convert(options)
    if options[:lsf]
      cmd = "bsub -w \"done(bwa_#{options[:job_number]})\" -o #{options[:log_file]}_convert_o.log -e #{options[:log_file]}_convert_e.log -q plus -J convert_#{options[:job_number]} java -Xmx5g -jar #{options[:picard_tools]}/SamFormatConverter.jar I=#{options[:sam_file]} O=#{options[:bam_file]} VALIDATION_STRINGENCY=LENIENT"
    else
  ...
  def self.mark_duplicates(options)
    if options[:lsf]
      cmd = "bsub -w \"done(sorting_#{options[:job_number]})\" -o #{options[:log_file]}_duplicates_o.log -e #{options[:log_file]}_duplicates_e.log -q plus -J duplicates_#{options[:job_number]} java -Xmx5g -jar #{options[:picard_tools]}/MarkDuplicates.jar I=#{options[:bam_file_sorted]} O=#{options[:bam_file_sorted_duplicates]} VALIDATION_STRINGENCY=LENIENT M=#{options[:duplicate_metrcis]} AS=true"
    else
  ...
  def self.build_index(options)
    if options[:lsf]
      cmd = "bsub -w \"done(duplicates_#{options[:job_number]})\" -o #{options[:log_file]}_index_o.log -e #{options[:log_file]}_index_e.log -q plus -J index_#{options[:job_number]} java -Xmx5g -jar #{options[:picard_tools]}/BuildBamIndex.jar I=#{options[:bam_file_sorted_duplicates]} VALIDATION_STRINGENCY=LENIENT"
    else
