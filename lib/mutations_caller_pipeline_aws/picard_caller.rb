class PicardCaller
  ...
  def self.convert(options)
    if options[:lsf]
      cmd = "bsub -w \"done(bwa_#{options[:job_number]})\" -o #{options[:log_file]}_convert_o.log -e #{options[:log_file]}_convert_e.log -q plus -J convert_#{options[:job_number]} java -jar #{options[:picard_tools]}/SamFormatConverter.jar I=#{options[:sam_file]} O=#{options[:bam_file]} VALIDATION_STRINGENCY=LENIENT"
    else
  ...
  def self.rg_and_sorting(options)
    if options[:lsf]
      cmd = "bsub -w \"done(bwa_#{options[:job_number]})\" -o #{options[:log_file]}_convert_o.log -e #{options[:log_file]}_convert_e.log -q plus -J convert_#{options[:job_number]} java -jar #{options[:picard_tools]}/AddOrReplaceReadGroups.jar I=#{options[:bam_file]} O=#{options[:bam_file_sorted]} VALIDATION_STRINGENCY=LENIENT SO=coordinate ID=#{options[:id]} LB=#{options[:library]} PL=Illumina PU=#{options[:index]} SM=#{options[:sample_name]} MAX_RECORDS_IN_RAM=1500000"
    else
      cmd = "qsub -o #{options[:log_file]}_convert_o.log -e #{options[:log_file]}_convert_e.log -V -cwd -b y -hold_jid bwa_#{options[:job_number]} -N convert_#{options[:job_number]} -l h_vmem=14G java -jar #{options[:picard_tools]}/AddOrReplaceReadGroups.jar I=#{options[:bam_file]} O=#{options[:bam_file_sorted]} VALIDATION_STRINGENCY=LENIENT SO=coordinate ID=#{options[:id]} LB=#{options[:library]} PL=Illumina PU=#{options[:index]} SM=#{options[:sample_name]} MAX_RECORDS_IN_RAM=1500000"
    end
    cmd
  end
