class GatkCaller
  ...
  def self.coverage(options)
    if options[:lsf]
      cmd = "bsub -n #{options[:threads]} -w \"done(recal_#{options[:job_number]})\" -o #{options[:log_file]}_coverage_o.log -e #{options[:log_file]}_coverage_e.log -q plus -J coverage_#{options[:job_number]} java -Xmx5g -jar #{options[:gatk]} -R #{options[:index_fa]} -T DepthOfCoverage -I #{options[:recal_bam]} -o #{options[:outfile_prefix]} --omitDepthOutputAtEachBase --omitIntervalStatistics --omitLocusTable -nt #{options[:threads]}"
    else
