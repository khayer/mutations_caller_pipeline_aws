...
def make_link(pos, location)
  ...
  pos_l = (pos - 10).to_s
  pos_r = (pos + 10).to_s
  link = "http://mar2015.archive.ensembl.org/Danio_rerio/Location/View?r=#{location}:#{pos_l}-#{pos_r}"
end
...
def make_link_ensembl(ensemble_id)
  link = "http://mar2015.archive.ensembl.org/Danio_rerio/Gene/Summary?db=core;t=#{ensemble_id}"
end
...
sheet1.row(0).push "#{target_name[0]} base",'Impact','Effect', 'Codon Change','Amino Change','Amino Change Detail', 'Gen Name','Phast Cons','Score'
...
File.open(vcf_file).each do |line|
  ...
  if line =~ /(HIGH|MODERATE)/
    ...
    #INTRON(MODIFIER||||BOD1L1|protein_coding|CODING|ENSDART00000129095|7|WARNING_TRANSCRIPT_NO_STOP_CODON
    ...
    amino_change = "n/a"
    #aa_change = ""
    if effect == "NON_SYNONYMOUS_CODING" || effect == "STOP_GAINED"
    ...
    sheet1.row(i).push impact,effect,codon_change,amino_change,aa_change,Spreadsheet::Link.new(link_transcript_id,gene_name)
  else
    sheet1.row(i).push 'n/a','n/a','n/a','n/a','n/a','n/a'
  end
