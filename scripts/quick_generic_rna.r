...
for (k in 1:25) {
    ...
    for (i in 2) { plot(Test[,1],Test[,i],type="l",col='cornflowerblue', xlab = "", ylab = "",xaxt = 's', ylim = c(-0.1,0.85))
                                par(new=T)}
    abline(h = 0.6, lty = 3, col = 'grey72')
    par(new=TRUE)
    ...
    title(main=sprintf("%s%i, window_length = %s SNP's, Homozygosity Score (Wik +TLF)", combo_sn2, k, ws),ylab="Homozygosity score")
